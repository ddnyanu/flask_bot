...
def generate_answer(question: str, context_chunks: List[dict]):
    ...
    try:
        ...
        try:
            detected_lang = single_detection(english_cleaned, api_provider='google')
            if detected_lang == 'hi':
                english_cleaned = GoogleTranslator(source='hi', target='en').translate(english_cleaned)
        except:
            pass
        ...
        hindi = GoogleTranslator(source='en', target='hi').translate(english_cleaned)
        return english_cleaned + english_source_text, hindi + hindi_source_text
    except Exception as e:
        return f"Answer generation failed: {e}", ""
